...
import com.aqm.staf.library.databin.ClientDetailsEntity;
import com.aqm.staf.library.databin.MembAtachCovrgPremSchdEntity;
import com.aqm.staf.library.databin.PartyAgentPartyRuleEntity;
...
public class PartyKeyWord extends FunctionalKeywords{
						...
						if(partyEntity.getParentKey().equalsIgnoreCase(partyRolesFunctDetailsData.getChildKey())){
							if(partyRolesFunctDetailsData.getAction().equalsIgnoreCase("verify")&& partyRolesFunctDetailsData.getStepGroup().equalsIgnoreCase(stepGroup)){
								...
								if(partyRolesFunctDetailsData.getStringValueForField("FunctionTextField").equalsIgnoreCase("MEMBER") || partyRolesFunctDetailsData.getStringValueForField("FunctionTextField").equalsIgnoreCase("POLICY-HOL")){
									...
									List<PartyAgentPartyRuleEntity> PartyAgentPartyRuleEntityList=testData.getPartyAgentPartyRuleEntity();
	...
	private void setEditPartyDependency(PartyEntity partyEntity, ScenarioTestData testData) {
		...
		if(partyEntity.getStringValueForField("DependencyForClientDetail")!=null){
			ArrayList<ClientDetailsEntity> clientDetailListAll=testData.getObjectByReference(testData.getClientDetailsEntityRecords(), partyEntity.getStringValueForField("DependencyForClientDetail"));
			ArrayList<ClientDetailsEntity> clientDetailEntityList = FrameworkServices.getObjectByPrimaryKey(
					clientDetailListAll, partyEntity.getStringValueForField("DependencyForClientDetail"));

			for(ClientDetailsEntity clientDetailEntityData:clientDetailEntityList){
